...
class trade:
    ...
    def create_charts(self, stockName):
        # download stock data from yfinance into a pandas dataframe
        data = yf.download(tickers=f'{stockName}', period='1d', interval='5m')
    ...
    def calcBollAndRsi(data):
        # add additional columns necessary for bollinger bands strategy
    ...
    def bollingerBandRsiStrategy(data):
        ...
        choices = ['Buy', 'Sell']
        # buy or sell depending on which condition
        ...
        # go through stock dataframe and buy or sell stock depending on the postion and signal
        for i in range(len(data)):
            # meaning we have not bought yet and signal indicates buy
            if not position and data['Signals'][i] == 'Buy':
                buyDate.append(data.index[i])
                buyPrice.append(data.Open[i])
                sellPrice.append(np.nan)
                position = True
            # we have bought and signal indicates sell 
                ...
                buyPrice.append(np.nan)
                sellPrice.append(np.nan)
            # continously track our position to calculate cumulative returns 
            if not position:
                signal.append(0)
            else:
                signal.append(1)
    ...
    def calcProfits(data, buyDate, sellDate):
        # calcuate cumulative returns based on the strategy
        buys = data.loc[buyDate].Open
        sells = data.loc[sellDate].Open
        profits = (pd.Series([(sell - buy) / buy for sell, buy in zip(sells, buys)])+1).prod()-1
        return profits

    def bollinger(self, stockName, interval):
        # check valid stock ticker
        try:
        ...
        buyDate, sellDate, buyPrice, sellPrice, signal = trade.bollingerBandRsiStrategy(data)
        ...
        # visualize upper and lower bands alongs with performance
